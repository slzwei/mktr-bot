...
Status: todo
Evidence: 2026-09-11 audit: `server/telephony.ts:82` creates a socket per command and closes it at line 90; `server/esl.ts`, persistent event subscription/reconnect, and provider-event lifecycle handling are absent; playback still advances by timer at `server/orchestrator.ts:240`. `npm test` passes 4/4 existing tests, with no fake ESL handshake/body/error/reconnect/hangup-slot tests; this is not verification against fake ESL.
...
Status: todo
Evidence: 2026-09-11 audit: `telephony/freeswitch/conf/` and the render script are absent; compose mounts only gateway XML/certs (`docker-compose.yml:73`), with no vars/switch overlay, CA installation as cafile.pem, public-IP rendering, or matched RTP configuration. `npm run render:freeswitch` reports Missing script; image pullability is unverified. `docker compose --profile live config --quiet` cannot run because Docker is absent; real registration is not verifiable here. After implementation, the operator must use a Singtel-whitelisted gateway with credentials/CA and run `fs_cli -x "sofia status gateway singtel"` for REGED and `fs_cli -x "sofia status profile external"` for TLS 5061; fs_cli is also absent here.
...
Verify: An e2e test uploads a 44.1 kHz stereo MP3 and asserts the FreeSWITCH file is 8 kHz mono WAV with duration within 1 s of the probe. A malformed flow PUT returns 400.
