...
### B6. Delivery hygiene
Status: doing
Evidence: main is published to https://github.com/slzwei/mktr-bot and first default-branch CI https://github.com/slzwei/mktr-bot/actions/runs/34568053353 is green, including build/typecheck, 99 unit tests, 31 media tests, eight Compose Postgres tests, backup restoration, 24 Chromium tests, restart recovery, Compose validation and both application image builds/non-root API UID. Additional isolated container startup/authentication/health/shutdown verification is implemented in npm run test:containers and pending its GitHub Linux run; local invocation reports Docker not installed on host.
