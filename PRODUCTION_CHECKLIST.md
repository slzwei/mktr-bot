...
Status: blocked
Evidence: Implementation complete: pinned-source FreeSWITCH with mod_audio_stream/mod_avmd and Node 24.14.0, 13 dummy XML renders and all configuration tests pass in the 99-test suite; build and 24 Chromium tests pass. Docker-dependent config/build Verify blocked: Docker not installed on host. Real REGED/TLS Verify blocked pending Shawn’s Singtel-whitelisted Linux host, SIP credentials, supplied CA, matching gateway TLS certificate/private key and private fs_cli diagnostics; docs/freeswitch-deployment.md records the inputs. No live registration or call attempted.
...
### B6. Delivery hygiene
Status: blocked
Evidence: Implementation and available checks complete: build, 99 unit tests, 24 Chromium tests, npm run test:coverage (94.30% aggregate lines), native Postgres backup/restore drill and npm audit (0 vulnerabilities) pass. Node 24.14.0 is pinned across images; fresh production output contains no tests; CI uploads only failure screenshots; names-only env startup/sign-in/shutdown passes over HTTP. Verification blockers: no GitHub remote/default-branch CI run is configured; Docker-dependent image build/UID/compose checks blocked: Docker not installed on host. See docs/verification-2026-09-11.md.
