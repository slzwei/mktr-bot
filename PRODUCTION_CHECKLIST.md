...
Status: done
Evidence: Local `npm run lint`, `npm test` (116/116, including 16 fake-gateway DNC tests), `npm run build` and `npx playwright install chromium && npm run test:e2e` (28/28) pass. `npm run test:db:local` passes 9/9, including S000 JSON evidence and append-only enforcement across restart. Tests prove zero writes for non-S000/partial replies, disabled import compatibility, confirmed credit limits and selectable registered-row UI; preview/permission screenshots were inspected. Compose YAML/default-off wiring checked; Docker unavailable. Conformance-checked against the built mktr-platform gateway: absent PDPC `createdTime`/`validUntil` no longer discard a paid batch, and the request timeout now exceeds the gateway's ~35s advisory-lock worst case. Provisioned and activated 2026-09-11: mktr-platform gateway merged (#522) and live at `POST /api/external/dnc-batch` with a dedicated 64-hex secret and a 500-credit hourly cap; `MKTR_DNC_ENABLED=true` on mktr-voice with the same secret. End-to-end wire proven at zero cost — a signed request from the droplet was HMAC-verified by the gateway and rejected at validation (400), and a valid-number request against a pinned-zero budget returned 429 rather than 503, proving the PDPC credential set is live. Still no live Registry call: the first paid check (2 credits) is the operator's, from the UI, with the credit count displayed.
