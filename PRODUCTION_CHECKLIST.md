...
### B6. Delivery hygiene
Status: doing
Evidence: First default-branch CI34568053353 is green for build/typecheck, 99 unit/31 media/eight Compose Postgres tests, backups, 24 Chromium tests, restart recovery, Compose validation and both image builds. Enhanced run34569318116 confirms migrations, healthy images and non-root Node PID1, then exposes Docker’s intentional lack of host publication on an internal network. HTTP probes now execute inside the API fixture and reach the worker through its internal alias; the final authentication/shutdown checks await the corrected Linux run. No real provider or call is involved.
