...
Status: blocked
Evidence: Pinned-source TLS/SRTP overlay and 13 dummy XML renders pass; default-branch CI https://github.com/slzwei/mktr-bot/actions/runs/34568053353 validates Compose and builds API/worker images. A separate build-only Linux gateway workflow now verifies the pinned FreeSWITCH image; its first result is pending. Local Docker is absent. Real REGED/TLS remains blocked on Shawn’s whitelisted host, SIP credentials, CA, matching gateway TLS certificate/key and private fs_cli evidence. No FreeSWITCH process, registration or call was started.
