...
Status: done
Evidence: `npm run test:media` passes 31 loopback/fake-STT/Deepgram-wire tests including one stable utterance receipt, HTTP retries after audio closure, failed provider open/write/close, abort and late-open disposal, upgrade/body/buffer/lifetime bounds, five active audio slots, malformed frames and final-word latency estimates. Merged build, 99 unit tests and 24 Chromium tests pass. Verified against fake ESL and fake STT. Docker-dependent service/healthcheck Verify blocked: Docker not installed on host; implementation and all available checks complete under the task's Docker exception. Real provider/Singtel behavior remains operator-only.
...
Status: blocked
Evidence: TLS/SRTP overlay and 13 dummy XML renders pass; default-branch CI https://github.com/slzwei/mktr-bot/actions/runs/34568053353 validates Compose and builds API/worker images. The first gateway image run34568857931 exposed a missing PCRE2 dependency; builder/runtime packages now match the pinned configure.ac requirement, with the corrected image awaiting its Linux rebuild. Real REGED/TLS remains blocked on Shawn’s whitelisted host, SIP credentials, CA, matching TLS certificate/key and private fs_cli evidence. No gateway process, registration or call was started.
...
Status: blocked
Evidence: Runbook and private operator helpers implemented; npm run verify:runbook passes 14 Bash blocks/6 inline references, 13 dummy XML renders, 11 fake-Docker diagnostics including mod_avmd, namespace fallback/originate rejection and first-call configuration fixtures. Missing: Shawn’s dated review and designation of the single approved E.164 destination. Docker-dependent functional verification separately blocked: Docker not installed on host. No real registration, provider connection or call attempted; operator checks are explicit in docs/runbook-first-live-call.md.
...
Status: done
Evidence: npm run test:db:local passes 8/8 against disposable PostgreSQL 17, including authenticated API restart/session/flow/clip persistence, immutable versions, transactional failure, campaign state, append-only consent and durable webhook attempts. Orchestrator flushes before provider effects/SSE; the delayed-disk test proves five reservations with no premature originates. Build, 99 unit tests and 24 Chromium tests pass. Compose-based npm run test:db Verify blocked: Docker not installed on host; the same real Prisma suite passes natively under the task’s Docker exception.
...
### B6. Delivery hygiene
Status: doing
Evidence: Published main and first CI run34568053353 are green for build, 99 unit/31 media/eight Compose Postgres tests, backup restoration, 24 Chromium tests, restart recovery, Compose validation and API/worker image builds/non-root UID. Enhanced run34568857946 started both simulator images and applied all four migrations, then exposed a test assumption: Node24 names its Linux main thread MainThread. The PID1 check now compares /proc/1/exe with the Node executable; final runtime health/authentication/shutdown verification awaits the corrected Linux run. Local Docker remains absent.
