...
export function permissionDisplay(permission?: ContactPermission) {
  if (!permission) return { tone: "blocked", label: "Permission unavailable", detail: "Permission status could not be loaded." };
  ...
  if (permission.basis === "dnc" && permission.clearanceExpiresAt && Date.parse(permission.clearanceExpiresAt) - Date.now() <= 3 * 24 * 60 * 60 * 1000) {
    return { tone: "expiring", label: `Dialable · clearance expiring ${new Date(permission.clearanceExpiresAt).toLocaleString("en-SG", { timeZone: "Asia/Singapore" })} Singapore time`, detail: "Registry clearance expires within three days." };
  }
