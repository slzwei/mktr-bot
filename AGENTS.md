...
- `npm run test:coverage` runs the same behavioural unit suites with Node's coverage report
- `npm run test:backup` runs a disposable local Postgres backup/restore drill with `initdb`, `pg_ctl`, `psql`, `pg_dump`, and `pg_restore`
- `npm run test:containers` requires the built `mktr-voice-control:ci` and `mktr-media-worker:ci` images plus Docker; it verifies startup, authentication, health and clean shutdown on an isolated simulator network with disposable Postgres, and removes only its own fixtures.
