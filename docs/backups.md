...
On 11 September 2026 this drill passed on native PostgreSQL 17.10 on this host and native PostgreSQL 16 in [GitHub CI run 34569980907](https://github.com/slzwei/mktr-bot/actions/runs/34569980907): a disposable localhost cluster backed up and restored relational `Flow`/`Call` rows and clip bytes, checked the restored join result and byte equality, and rejected nonempty database/directory targets and a checksum mismatch. Each temporary cluster was stopped and deleted. The same CI run also passed eight Prisma tests against Compose PostgreSQL 16. These establish the real dump/restore toolchain and archive checks; Shawn must still drill the deployed database and actual mounted clip volume before relying on its backups.
