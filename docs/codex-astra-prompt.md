...
Two prompts. Use the build prompt to do the work. Use the audit prompt to check the repo against the checklist without changing code, either before the build run or after it.

Run Codex from the repo root so it picks up `AGENTS.md`. Set reasoning to the highest level available. No credentials are needed; the run stays in simulator mode.
...
- Verified state on 11 Sep 2026: npm run build, npm test (4 unit tests) and npm run test:e2e pass, and npm audit is clean. Simulator mode works end to end. Live mode cannot work yet: there is no media worker or speech-to-text, the ESL adapter is fire-and-forget, the FreeSWITCH config will not register with Singtel, there is no authentication, and the store is in memory.
...
Reserved for the operator. Do not do these. List them at the end as operator actions: placing any real call, enabling freeswitch mode against real credentials, contacting Singtel, choosing or registering a DNC Registry account, buying anything, setting up the production host.
...
- The folder is not a git repo. Run git init first, commit the current state as "Baseline: simulator and flow editor", then commit after every checklist item with the item ID as the prefix, for example "A2: persistent ESL event client". Never use git reset --hard or discard changes you did not make.
...
- Delegate to parallel sub-agents where items are independent, and say in the report which items you delegated:
  ...
  - Sub-agent 2: A7 (auth, Caddy, hardening), then B6 (git hygiene, CI, Dockerfile, backups).
...
- Do not run docker compose --profile live. docker compose config and docker compose --profile live config are the only compose commands you need.
