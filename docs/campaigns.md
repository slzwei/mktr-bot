...
Authenticated routes are `GET /api/contacts`, `POST /api/contacts/import`, `GET /api/campaigns`, `POST /api/campaigns`, `GET /api/campaigns/:id`, and `POST /api/campaigns/:id/:action` (`start`, `pause`, `stop`). Import accepts `{csv}`; controls accept `{}`. Creation accepts `name`, `flowId`, `callerId`, `contactIds` and optional `flowVersion`, `callingHours`, `maxAttempts`, `retryDelaySeconds`, `dialIntervalMs`. Timestamps are stored in UTC; calling hours and displayed next attempts use Singapore time.
