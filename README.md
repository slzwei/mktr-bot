...
**Live calls**, beside Call history in the workspace navigation, watches concurrent sessions from the existing recent-call roster. Each card shows the contact or destination, caller ID, campaign, flow/version, current node, status, elapsed duration, recording state and a live technical timeline in Singapore time. Captured caller speech appears below the timeline; clip events identify playback without claiming to transcribe the agent audio. Each running call has its own existing SSE connection. The capacity banner explains when campaigns must wait for a slot. End stops only the selected call; completed and failed cards retain their outcome and end reason in place until dismissed or you leave this view. Full records remain in Call history, and the flow editor and history still open the single-call console.

Timers and logs can be paused while status and End controls stay live; reduced-motion preferences start with them paused. Logs and timers do not announce every update to screen readers, and timeline scroll regions are keyboard reachable. Names use the existing contact/campaign lookups. A missing or changed flow version shows the node ID and an unavailable-label explanation. The roster is limited to the 25 most recent calls; if older calls still occupy capacity, the view states that the displayed roster is incomplete.
...
Call updates include event IDs and a comment heartbeat every 15 seconds. The console and Live calls share native EventSource reconnection and refetch the current call whenever a stream opens again; permanently closed transports are recreated after one second. Last snapshots stay visible during reconnects, and late reads cannot resurrect a finished session or shorten a newer timeline. Operational state refreshes every two seconds and on window focus so another operator's activity reaches the capacity meter without overwriting an unsaved flow. SSE snapshots update the local roster directly without triggering additional bootstrap reads. Streams close on completion, roster removal, or view unmount; closing the console also releases its stream. Caddy flushes `/api/calls/*/events` immediately; the API disables intermediary buffering and releases each subscription on disconnect.
...
The FreeSWITCH adapter keeps a persistent authenticated ESL connection. Answer, hangup, originate results and playback completion drive the orchestrator; flow completion and errors terminate the provider channel. Reconnect reconciles channels and never replays originate commands. At answer, the adapter starts one callee audio stream for the whole call, and the media worker verifies the call through the bearer-protected API and holds one Deepgram connection until the channel clears. The orchestrator opens and closes each listen window on the worker over the same private network; the worker submits one final transcript per window with stable window/utterance receipt IDs and discards speech finalized outside one. The API classifies the transcript, selects the published flow route and plays its canonical clip. Answer confirmation comes from ESL.
...
The separate `media-worker` receives one authenticated callee-only PCM WebSocket from `mod_audio_stream` per answered call whose flow listens, and keeps it and its provider connection open for the whole call so no listen window pays a connection handshake. `MKTR_STT_PROVIDER=deepgram` selects Nova-3 over linear16 8 kHz mono at Deepgram's Sydney origin; `MKTR_DEEPGRAM_BASE_URL` (default `https://api.au.deepgram.com`, a bare https origin) selects another region. Endpointing, the silence after the caller stops before the reply is final, is set per listen node in the flow editor (default 300 ms, range 100–1000 ms); the worker opens its connection with the first listen node's value and reconnects only for a node that uses a different one. `MKTR_STT_LANGUAGE` defaults to `en-SG` and maps to Deepgram wire code `en` (the provider does not list `en-SG`). Configure `DEEPGRAM_API_KEY` and a URL-safe `MKTR_MEDIA_GATEWAY_TOKEN` of at least 16 characters privately on the operator host. Simulator mode leaves provider streaming disabled. Run `npm run test:media` for loopback audio and fake STT verification; real speech accuracy belongs to the first-live-call runbook. `MKTR_PCM_CAPTURE_ENABLED=true` makes the worker keep the exact callee PCM and arrival timing of each whole call under an ignored directory, and `npx tsx scripts/replay-stt.ts` replays a capture into a chosen engine at the recorded pacing to time end of speech to transcript; see [media worker behavior](docs/media-worker.md).

One provider connection serves every listen window in a call, and the worker tells the API as soon as the caller is heard saying words so a listen node's silence timeout does not fire on someone mid-sentence. The worker cancels and bounds provider connections, accepts one reply per listen window, and retries lost transcript replies using the same receipt ID. Call capacity is released with the channel, independently of pending HTTP delivery. STT latency uses the final word's audio timestamp; unknown timing is omitted. See [media worker behavior](docs/media-worker.md) for lifecycle limits, receipt handling, and the estimate's precision.
