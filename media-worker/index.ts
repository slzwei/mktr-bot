...
const worker = createMediaWorker({
  ...
  stt: new DeepgramSpeechToText({ apiKey: process.env.DEEPGRAM_API_KEY ?? "", language: process.env.MKTR_STT_LANGUAGE || "en-SG", endpoint: deepgramEndpoint, ...(model === undefined ? {} : { model }) }),
...
worker.server.listen(port, "0.0.0.0", () => logger.info({ port, enabled, provider, deepgramEndpoint, model: model ?? deepgramDefaults.model, maxCallSeconds, pcmCapture: captureDirectory ?? false }, "Media worker ready"));
let stopping = false;
