...
// Opt-in replay corpus: raw callee PCM per listen window plus arrival timing, for scripts/replay-stt.ts.
...
worker.server.listen(port, "0.0.0.0", () => logger.info({ port, enabled, provider, deepgramEndpoint, pcmCapture: captureDirectory ?? false }, "Media worker ready"));
let stopping = false;
